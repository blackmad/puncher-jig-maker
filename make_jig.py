...
class PuncherMaker(BaseRulerMaker):
    ...
    def make_belt_jig(self, height, ignore):
        ...
        self.draw_circle_at_center(width / 2 - BeltHoleWidth / 2, height / 2,
                                   FiveMM)
        self.draw_circle_at_center(width / 2 + BeltHoleWidth / 2, height / 2,
                                   FiveMM)
        self.set_cut_line()
        self.pdf.rect(width / 2 - BeltHoleWidth / 2, height / 2 - FiveMM / 2,
                      BeltHoleWidth, FiveMM)
    ...
    def make_jig(self, height, width):
        ...
        if (height / 3 > FiveMM):
            right_slices = 3
        self.draw_at_slice_right(slices=right_slices,
                                 width=width,
                                 height=height)
    ...
    def draw_ruler(self, width, height, margin=0, invert=False, bold=False):
        ...
        if margin == 0:
            ...
            SpecialSlices = [
                first_special_slice,
                first_special_slice + special_offset_slice,
                slices - 2 - first_special_slice,
                slices - 2 - first_special_slice - special_offset_slice
            ]
        else:
    ...
    def draw_at_slice_left(self, slices, width, height):
        for fraction in range(1, slices):
            ...
            # now draw circlesg
            self.make_circles_at(x=FirstHoleAt,
                                 y=y,
                                 size=FiveMM,
                                 spacing=spacing,
                                 num=int(
                                     ((width / 2) - FirstHoleAt) / spacing))
    ...
    def draw_at_slice_right(self, slices, width, height):
        for fraction in range(1, slices):
            ...
            # now draw circles
            self.make_circles_at(x=width - FirstHoleAt,
                                 y=y,
                                 size=FiveMM,
                                 spacing=spacing,
                                 num=int(((width / 2) - FirstHoleAt) /
                                         math.fabs(spacing)))
...
def make_jig_helper(cbname, filename):
    ...
    while (size < max_size):
        ...
        cb2(size, width)
        pdf2.output('%s-%s_%s' % (width, size, filename))

        size += interval

    pdf.output('%s-%s' % (width, filename))
